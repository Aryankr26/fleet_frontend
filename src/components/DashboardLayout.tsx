  ...
  return (
    <div className="flex min-h-screen bg-white overflow-auto">
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 bg-[#0a0f1e] text-white p-3 rounded-lg shadow-lg hover:bg-[#1a1f2e] transition-colors"
      ...
      <aside className="hidden lg:flex relative z-10 w-72 bg-[#0a0f1e] text-white flex-col shadow-2xl">
        {/* Logo Section */}
      ...
      <Sheet open={isMobileSidebarOpen} onOpenChange={setIsMobileSidebarOpen}>
        <SheetContent side="left" className="w-72 p-0 bg-[#0a0f1e] text-white border-r-0">
          {/* Logo Section */}
          <div className="p-6 border-b border-slate-800">
            <div className="flex items-center gap-3">
              <div className="bg-[#10b981] p-2 rounded-lg">
      ...
      {currentPage === 'dashboard' && (
        <main className="relative z-10 flex-1 overflow-auto min-h-screen w-full lg:w-auto">
      ...
      {currentPage === 'vehicles' && (
        <main className="relative z-10 flex-1 overflow-auto min-h-screen w-full lg:w-auto">
          <div className="p-0 md:p-4 lg:p-6">{children}</div>
        </main>
      )}
      ...
      <nav className="lg:hidden fixed bottom-4 left-1/2 -translate-x-1/2 z-50 bg-white/90 backdrop-blur-lg rounded-full shadow-lg px-3 py-2 flex items-center gap-2">
        {navItems.slice(0,5).map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => handleNavigation(item.id)}
              className={`flex flex-col items-center justify-center px-3 py-2 rounded-full ${isActive ? 'bg-blue-600 text-white' : 'text-slate-700'}`}
