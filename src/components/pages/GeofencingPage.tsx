...
import mapBackground from '../../assets/84f2df699de86d311dc0b56820be473ca8ac952c.png';
    ...
    <div className="relative w-full h-screen overflow-hidden">
      ...
      >
        ...
        {selectedPoint && (
          ...
          >
            <div className="relative">
              {/* compute circle size based on selected radius (responsive) */}
              {(() => {
                const mapRadius = radius === '100' ? 40 : radius === '250' ? 80 : radius === '500' ? 120 : 160; // px at default zoom
                const size = Math.max(24, Math.round(mapRadius * (1 / mapZoom))); // scale with zoom
                return (
                  <div
                    style={{
                      width: `${size}px`,
                      ...
                      boxShadow: '0 0 0 8px rgba(59,130,246,0.06) inset'
                    }}
        ...
        {vehicles.map((vehicle) => (
          ...
          >
            <div className="relative">
              <div className={`${vehicle.status === 'inside' ? 'bg-green-500' : 'bg-red-500'} p-2 md:p-3 rounded-full shadow-xl animate-pulse cursor-pointer`}>
                <Navigation className="h-4 w-4 md:h-5 md:w-5 text-white" />
              </div>
              
              {/* Vehicle Tooltip - becomes stacked and wider on small screens */}
              {hoveredVehicle === vehicle.id && (
                <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 bg-white px-3 py-2 rounded-lg shadow-2xl z-50 whitespace-normal w-56 md:w-auto">
                  <p className="text-xs text-slate-900 truncate">{vehicle.id}</p>
      ...
      {/* Zoom Controls */}
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 sm:left-auto sm:translate-x-0 sm:right-6 flex flex-row sm:flex-col gap-2 z-30">
        <button
          onClick={handleZoomIn}
          className="w-10 h-10 sm:w-12 sm:h-12 bg-white hover:bg-slate-50 rounded-lg shadow-lg flex items-center justify-center transition-all hover:scale-105 active:scale-95"
          title="Zoom In"
        >
        ...
        <div className="w-12 bg-white rounded-lg shadow-lg px-2 py-2 text-center hidden sm:block">
      ...
      {showAddGeofence && (
        ...
        >
          <div 
            className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[95vh] overflow-hidden mx-2 sm:mx-6"
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Input
                        ...
                        placeholder="1"
                        value={radiusValue}
                        onChange={(e: any) => setRadiusValue(e.target.value)}
